...
function ActiveFilterButton(props: ActiveFilterButtonProps) {
    const { onChange, iconClass, title, arrowIconElement, buttonText, onClear, innerRef, colum } = props
    ...
    const getViewValueForFilter = (buttonText: string) => {
        const newString = buttonText?.split(",")?.map((item: string) => {
            if (typeof colum?.options?.optionSet !== "undefined" && item !== "") {
                item = colum?.options?.optionSet?.options?.find((option: any) => option?.value === item)?.label
            }
            return item;
        })
        buttonText = newString.join(",");
